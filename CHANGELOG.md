...
- `halt_after:` (wrap the success payload in `RubyLLM::Tool::Halt`), `provider_params:` (forwarded to `with_params`), and `render_as:` (`:structured` default vs `:text`, returning `result.message`) are settable per-call or once on the Axn via `set_extension_metadata(:ruby_llm, ...)`, with a `wrap` keyword taking precedence.
