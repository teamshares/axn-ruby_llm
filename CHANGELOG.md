...
- `halt_after:` (wrap the success payload in `RubyLLM::Tool::Halt`), `provider_params:` (forwarded to `with_params`), and `present_as:` (`:structured` default vs `:message`, returning `result.message`) are declared as overridable settings under the `:ruby_llm` config namespace (axn's namespaced per-class config, PRO-2880/[axn#154](https://github.com/teamshares/axn/pull/154)) — settable per-call via a `wrap` keyword, once on the Axn via `configure(:ruby_llm) { |c| ... }` (no `include` required — every Axn gets `configure` for free), or gem-wide via `Axn::RubyLLM.configure { |c| ... }`, in that precedence order. Namespacing is what lets one base Axn be configured for both this gem and axn-mcp (or any other adapter) without their settings colliding, even on a shared name. `present_as:` deliberately shares its name and values (`:structured` / `:message`) with axn-mcp's own `present_as:`, so `configure(:ruby_llm) { |c| c.present_as = :message }` and `configure(:mcp) { |c| c.present_as = :message }` are the same knob. (This setting was briefly named `render_as:` with a `:text` value during development; that spelling now raises a pointed `ArgumentError` — see [DEPRECATIONS.md](DEPRECATIONS.md).)
