...
`input_tokens` / `prompt_tokens` keep returning their old values (no warning; exposures have no hook for one) — the bare `input_tokens` name meant *uncached* input here but *all* input in OpenTelemetry's conventions, so it's retired rather than redefined.
...
- Delete the `input_tokens` / `prompt_tokens` exposures and their entries in `usage_exposures` / `stubbed_exposures` (`lib/axn/ruby_llm/ask.rb`), plus the "keeps the deprecated fields' values" specs in `spec/axn/ruby_llm/ask_spec.rb`.
